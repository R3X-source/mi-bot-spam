...
const ID_CANAL_RECORDATORIO = "1481516697327243506";
...
function crearBot(token, num) {
    ...
    client.on('ready', () => {
        ...
        atacar(client); 
        // Solo el primer bot manda el recordatorio para no spamear la consola/canal de más
        if(num === 1) recordatorioEspecial(client);
    });
    client.login(token).catch(() => {});
}

// --- 📢 EL RECORDATORIO QUE PEDISTE ---
async function recordatorioEspecial(bot) {
    setInterval(async () => {
        try {
            const canal = await bot.channels.fetch(ID_CANAL_RECORDATORIO).catch(() => null);
            if (canal) {
                await canal.send(`⚠️ **RECORDATORIO:** Mandar 40 mensajes largos en este canal.`).catch(() => {});
            }
        } catch (e) {}
