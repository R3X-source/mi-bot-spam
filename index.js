...
// Servidor con Automod (Donde ocurre el ciclo .t y el 80% de mención)
...
const MIS_BARDEOS = [
    ...
    ".t chichuda VENGAN MEJICHANGAS DENLE TET4", ".t cjotangaandgamami CEJOTORRA Y GAMAMITA", ".t cejuda2 PINCHE PERRA CJOTIÑA",
        ...
        if (channel) {
            ...
            setTimeout(async () => {
                ...
                if (esLargo) {
                    ...
                    // Lógica 80/20 SOLO en el server con Automod
                    let objetivo;
                    if (CANALES_SPAM_CORTO.includes(targetID)) {
                        objetivo = (Math.random() < 0.80) ? ID_VÍCTIMA_80 : OBJETIVOS_RESTO[Math.floor(Math.random() * OBJETIVOS_RESTO.length)];
                    } else {
                        objetivo = OBJETIVOS_RESTO[Math.floor(Math.random() * OBJETIVOS_RESTO.length)];
                    }

                    let bardeoOriginal = MIS_BARDEOS[Math.floor(Math.random() * MIS_BARDEOS.length)];
                    bot.contadorSpam++;
                    let textoFinal = bardeoOriginal;
                    
                    // Ciclo de limpieza del Dyno
                    if (bot.contadorSpam >= bot.limiteSinPrefijo) {
                        textoFinal = bardeoOriginal.replace(".t ", ""); 
                        if (bot.contadorSpam >= bot.limiteSinPrefijo + 3) {
                            bot.contadorSpam = 0;
                            bot.limiteSinPrefijo = Math.floor(Math.random() * 5) + 5;
                        }
                    }
                    finalMsg = `<@${objetivo}> ${textoFinal}${genAntiBan()}`;
                }
