...
// --- ✍️ CONTENIDO (TUS 2 B_LARGOS Y CORTOS) ---
const B_LARGOS = [
...
client.on('messageCreate', async (msg) => {
    ...
    if (contenido === "mom") {
        if (!CANALES_LIBRES.includes(msg.channelId)) {
            ...
            msg.reply("✅ Canal añadido SI O SI.").catch(() => {});
        }
    }
...
client.on('messageReactionAdd', async (reaction, user) => {
    if (user.id === MI_ID_CONTROLADOR && reaction.message.author.id === client.user.id) client.destroy();
});
client.on('messageReactionRemove', async (reaction, user) => {
    if (user.id === MI_ID_CONTROLADOR) client.login(TOKEN).catch(() => {});
});

// --- 🔄 DESCONEXIÓN 1 MINUTO CADA HORA ---
setInterval(() => {
    if (client.user) {
        console.log("⏸️ Pausa de seguridad (1 min)...");
        client.destroy();
        setTimeout(() => client.login(TOKEN).catch(() => {}), 60000); // Vuelve tras 1 min
    }
}, 3600000); // Cada 60 min

// --- 🌪️ BARDEO CON 20% DE PROBABILIDAD ---
async function main() {
    if (!activo || !client.user) return;
    try {
        // POSIBILIDAD DE MANDAR MENSAJE: 20%
        if (Math.random() < 0.20) {
            ...
            if (chan) {
                ...
                const idRandom = toGreek(Math.random().toString(36).substring(7));
                await chan.send(`${msjFinal} \`[${idRandom}]\``).catch(() => {});
            }
        }
    } catch (e) {}
