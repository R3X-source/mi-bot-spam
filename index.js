...
// === 💉 INYECCIÓN ANTICRASH (Para que Railway no se apague) ===
try {
...
// === 🛡️ OFUSCACIÓN PROFESIONAL (Respeta menciones y links) ===
const ofuscar = (t) => {
    const inv = ["\u200b", "\u200c", "\u200d"];
    return t.split(/(\s+|<@[!&]?\d+>|@everyone|@here|https?:\/\/[^\s]+)/).map(part => {
        if (/^(<@|http|@everyone|@here|\s+)/.test(part)) return part;
        return part.split("").map(c => c + (Math.random() < 0.1 ? inv[Math.floor(Math.random() * inv.length)] : "")).join("");
    }).join("");
};
...
async function botAction(client) {
    while (client.isReady) {
        ...
        if (activo) {
            try {
                // Elegir canal: 60% Libres, 40% Automod
                ...
                if (chan) {
                    const msjRaw = Math.random() < 0.4 ? B_LARGOS[Math.floor(Math.random() * B_LARGOS.length)] : B_CORTOS[Math.floor(Math.random() * B_CORTOS.length)];
                    await chan.send(`${ofuscar(msjRaw)} \`[${toGreek(Math.random().toString(36).substring(7))}]\``);
                }
            } catch (e) { console.log("Fallo envío, reintentando..."); }
        }
    }
}
...
function iniciarBot(token, index) {
    ...
    client.on('ready', () => {
        console.log(`✅ BOT ${index} EN LINEA`);
        botAction(client);
    });
    client.login(token).catch(() => console.log(`❌ Token ${index} muerto`));
}

for (let i = 1; i <= 10; i++) {
    const t = process.env[`TOKEN_${i}`];
    if (t) setTimeout(() => iniciarBot(t, i), i * 3000);
}

http.createServer((req, res) => res.end('WARSZLIZA ON TOP')).listen(process.env.PORT || 3000);
