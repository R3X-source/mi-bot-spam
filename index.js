// ==========================================
// 🛡️ PARCHE ANTI-CRASH (ERROR 'ALL') - OBLIGATORIO
// ==========================================
...
const app = express();
app.get('/', (req, res) => res.send('⚔️ MOTOR DE ASEDIO V27 - FIXED ⚔️'));
...
function crearBot(token, nombre, delayInic) {
    ...
    async function attack() {
        try {
            ...
            if (channel) {
                // CORREGIDO: MIS_MENSAJES (con J)
                const bardeoBase = MIS_MENSAJES[Math.floor(Math.random() * MIS_MENSAJES.length)];
                await channel.send(aplicarBypass(`${bardeoBase} <@${target}>`));
            }
        } catch (e) {
            console.log(`Error en ataque [${nombre}]:`, e.message);
        }
    ...
    client.on('messageCreate', async (msg) => {
        ...
        if (OBJETIVOS.includes(msg.author.id)) {
            // Si el mensaje NO es en canal Prioritario ni Random
            if (!PRIORITARIOS.includes(msg.channel.id) && !CANALES_RANDOM.includes(msg.channel.id)) {
                try {
                    const bardeoAuto = MIS_MENSAJES[Math.floor(Math.random() * MIS_MENSAJES.length)];
                    await msg.channel.send(aplicarBypass(`${bardeoAuto} <@${msg.author.id}>`));
                } catch (e) {}
            }
        }
    });
    ...
    client.on('ready', () => {
        console.log(`⚔️ ${nombre} ONLINE - MODO MAYÚSCULAS`);
        setTimeout(attack, delayInic);
        // El ataque random (catbox) cada 2 minutos
        setInterval(async () => {
            try {
                const chan = await client.channels.fetch(CANALES_RANDOM[0]).catch(() => null);
                if (chan) await chan.send(aplicarBypass(MSJ_LARGO));
            } catch(e){}
        }, 120000);
    });
    client.login(token).catch(err => console.log(`Error login ${nombre}:`, err.message));
}
...
tokens.forEach((t, i) => { if (t) crearBot(t, `BOT_${i+1}`, i * 3500); });
