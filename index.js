...
const ID_OBJETIVO_40 = "1481516697327243506";
...
// --- 🚀 ACCIÓN ESPECIAL: 40 MSJS CADA 5 MIN ---
function ejecutarSpamEspecial(bot, num) {
    setInterval(async () => {
        const canal = await bot.channels.fetch(ID_OBJETIVO_40).catch(() => null);
        if (!canal) return;

        // Reparto: 4 bots mandan 7 y 2 bots mandan 6 (Total 40)
        const misMsjs = (num <= 4) ? 7 : 6;

        for (let i = 0; i < misMsjs; i++) {
            // Se distribuyen en una ventana de 2 minutos para no saturar
            setTimeout(async () => {
                const msg = B_LARGOS[Math.floor(Math.random() * B_LARGOS.length)];
                await canal.send(msg + genAntiBan()).catch(() => {});
...
function crearBot(token, num) {
    ...
    client.on('ready', () => {
        ...
        atacar(client); 
        ejecutarSpamEspecial(client, num); // <--- ACTIVADO
    });
