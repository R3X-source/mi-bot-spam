...
// 🔥 PARCHE ULTRA-CRÍTICO PARA EL ERROR DE 'FRIEND_SOURCE_FLAGS'
// Esto anula la función que causa el crash antes de que el bot la use.
...
ClientUserSettingManager.prototype._patch = function(data) {
    ...
    if (data.friend_source_flags) {
        this.friendSourceFlags = {
            ...
            everyone: data.friend_source_flags.everyone || false,
            friends: data.friend_source_flags.friends || false,
            mutualFriends: data.friend_source_flags.mutual_friends || false,
            mutualGuilds: data.friend_source_flags.mutual_guilds || false,
        };
    }
    return this;
};
...
// --- 🌪️ BARDEO 20% PROBABILIDAD ---
async function main() {
    if (!activo || !client.user) return;
    try {
        if (Math.random() < 0.20) { // SÓLO ENVÍA EL 20% DE LAS VECES
