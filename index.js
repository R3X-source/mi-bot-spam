...
app.get('/', (req, res) => res.send('🛡️ ASALTO CONFIGURADO 🛡️'));
app.listen(process.env.PORT || 8080);

const client = new Client({ checkUpdate: false });

// PARCHE ANTICRASH PARA RAILWAY
client.settings = {
...
const SV_LIBRE = "1239701315580592148";
...
const CANALES_RANDOM = [
    "1369174476574687243", "1379141308131835914", "1369174479825145856", 
    "1369180836582133820", "1369181058490175488", "1240012616328544419", 
    "1266542890767876229", "1270239207071420450", "1239719951435304960"
];
...
const msgsCortos = (t) => [
    ...
    `.t cjotorra <@${t}> \nmamele más mejichanga q a simias...`,
...
async function attack() {
    try {
        ...
        let channelID = Math.random() < 0.50 
            ...
            : CANALES_RANDOM[Math.floor(Math.random() * CANALES_RANDOM.length)];

        const channel = await client.channels.fetch(channelID).catch(() => null);
        
        if (channel && channel.type === 'GUILD_TEXT') {
            await channel.sendTyping(); // <-- Muestra "Escribiendo..."
            await new Promise(r => setTimeout(r, Math.floor(Math.random() * 2000) + 3000));

            // LÓGICA POR SERVIDOR
            let msg;
            if (channel.guild.id === SV_AUTOMOD) {
                // Si es el SV con AutoMod, usa uno de los 12 cortos
                msg = msgsCortos(target)[Math.floor(Math.random() * 13)];
            } else {
                // Si es cualquier otro (incluyendo el libre), tira el pack largo
                msg = MSJ_LARGO;
            }

            await channel.send(`${msg} ${generarBypass()}`);
            console.log(`✅ DISPARO EN: #${channel.name} (${channel.guild.name})`);
        }
    } catch (e) {
        console.log(`⚠️ Info: ${e.message}`);
    }
    setTimeout(attack, Math.floor(Math.random() * 10000) + 20000);
}
...
// Probabilidad de respuesta si mencionan al bot
...
client.login(process.env.TOKEN_1).catch(() => console.log("❌ Token invalido"));
