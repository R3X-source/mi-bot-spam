...
const CANAL_ARTILLERIA_LARGA = "1239719951435304960";
    ...
    client.on('messageCreate', async (msg) => {
        if (OBJETIVOS_MENCION.includes(msg.author.id) && !msg.author.bot) {
            ...
            setTimeout(async () => {
                ...
                if (Math.random() < 0.3) bardeo = bardeo.replace(".t ", ""); // Auto-responder también alterna
        ...
        if (channel) {
            ...
            setTimeout(async () => {
                ...
                if (bot.msgCount >= bot.triggerBypass) {
                    finalMsg = `cjotiña \`[${rStr}]\``; // Mensaje de camuflaje sin .t
                    bot.msgCount = 0;
                    ...
                    if (targetID === CANAL_ARTILLERIA_LARGA) {
                        ...
                        // --- SISTEMA DE ALTERNANCIA .T Y TAGS ---
                        if (Math.random() < 0.25) {
                            // MODO SIN TAGS Y SIN .T
                            bardeo = bardeo.replace(".t ", ""); 
                            finalMsg = `${bardeo} \`[Δ-${Math.floor(Math.random()*999)}]\` \`${rStr}\``;
                        } else {
                            // MODO NORMAL CON .T Y MENCION
...
const tokens = [process.env.TOKEN_1, process.env.TOKEN_2, process.env.TOKEN_3, process.env.TOKEN_4, process.env.TOKEN_5, process.env.TOKEN_6, process.env.TOKEN_7, process.env.TOKEN_8];
