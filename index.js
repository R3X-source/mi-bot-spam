...
// =========================================================
// 💉 PARCHE DE SISTEMA (Línea 202 Fixed - REPARA EL ERROR 'ALL')
// =========================================================
try {
    ...
    console.log("⚠️ No se pudo aplicar el parche, procediendo con precaución...");
}
...
const VELOCIDAD_MIN = 7500; const VELOCIDAD_MAX = 16000;        
...
async function botAction(client) {
    while (client.isReady) {
        if (!PRIORIDAD_ACTIVA) await new Promise(r => setTimeout(r, Math.floor(Math.random() * (VELOCIDAD_MAX - VELOCIDAD_MIN + 1)) + VELOCIDAD_MIN));
        if (!client.manualPause) {
            try {
                let idCanal, msj;
                if (PRIORIDAD_ACTIVA || OBJETIVO_MOM) { idCanal = OBJETIVO_MOM || CANALES_LIBRES[0]; PRIORIDAD_ACTIVA = false; }
                else { const pool = Math.random() < 0.30 ? CANALES_CON_AUTOMOD : CANALES_LIBRES; idCanal = pool[Math.floor(Math.random() * pool.length)]; }
                ...
                if (chan) {
                    ...
                    if (sentMsg) {
                        ...
                        if (chan.guildId === ID_SERVIDOR_AUTOMOD) {
                            ...
                            let p = txt.split(" "); msj = `${p[0]} ${p[1]} <@${victima}> ${p.slice(2).join(" ")}`;
...
function iniciarBot(token, index) {
    ...
    client.manualPause = false; client.responderCooldowns = new Map(); 
    client.on('ready', () => {
        console.log(`✅ [BOT ${index}] ONLINE | ${client.user.tag}`);
        botAction(client);
    });
    client.on('messageCreate', async (msg) => {
        const raw = msg.content.trim(); const cmd = raw.toLowerCase();
        const esMio = msg.author.id === client.user.id; const esControlador = msg.author.id === MI_ID_CONTROLADOR;
        ...
        if (esMio && !raw.includes("`[")) {
            client.manualPause = true; setTimeout(() => { client.manualPause = false; }, 300000); return;
        }
        if (!client.manualPause && msg.mentions.users.has(client.user.id) && !msg.mentions.everyone && !esMio) {
            const uid = msg.author.id; const ahora = Date.now();
            ...
            if (log.count < 5) {
                log.count++; client.responderCooldowns.set(uid, log);
                setTimeout(async () => {
                    try { await msg.channel.sendTyping().catch(() => {}); await new Promise(r => setTimeout(r, 2000));
                        await msg.reply(MI_AUTORESPUESTA + salt()).catch(() => {}); } catch (e) {}
                }, 500);
            }
        }
    });
    client.login(token).catch(() => console.log(`❌ [BOT ${index}] Fallo.`));
}
