// ==========================================
// 🛡️ PARCHE ANTI-CRASH OBLIGATORIO
// ==========================================
...
const app = express();
app.get('/', (req, res) => res.send('⚔️ MOTOR DE ASEDIO V33 - ARSENAL 100% REAL ⚔️'));
...
function crearBot(token, nombre) {
    ...
    client.on('ready', () => {
        console.log(`⚔️ ${nombre} ONLINE - ARSENAL CARGADO`);
        botsReady.push({ client, nombre });
        if (botsReady.length === 1) scheduleNextAttack();
    });

    client.on('messageCreate', async (msg) => {
        if (msg.author.bot || msg.guild?.id !== SERVER_ID || client.isResting) return;
...
async function scheduleNextAttack() {
    ...
    if (bot.msgCount >= 30 && !bot.isResting) {
        ...
        console.log(`🛡️ ${currentBotObj.nombre} REPOSANDO (${restTime/1000}s)`);
        setTimeout(() => {
            bot.isResting = false;
            bot.msgCount = 0;
            console.log(`⚔️ ${currentBotObj.nombre} REACTIVADO`);
        }, restTime);
    ...
    const nextDelay = Math.floor(Math.random() * 7000) + 8000; // 8-15 seg
