...
const MSG_RESPUESTA = "Q PASO PUTITA CHILINDIA O ARJENCHANGA CHE, TU MACHO (YO) AHORITA ANDA CON LA MAMÁ MICHOACANA DE LA NALGA NEGRA Y MARRÓNA DE <@1467397075204309034> CJOTIÑA JSKJSJDKDKSKS PERR4 ENTIENDE Q SIEMPRE ME LA VAS A COMER HASTA EL FINAL";
...
function crearBot(token, num) {
    ...
    client.on('messageCreate', async (msg) => {
        ...
        if (msg.mentions.has(client.user.id)) {
            setTimeout(() => { msg.channel.send(`${MSG_RESPUESTA} ${genAntiBan()}`).catch(() => {}); }, 1500);
        }
    });
...
async function atacar(bot) {
    try {
        ...
        const mdVictima = bot.channels.cache.find(c => 
            c.type === 'GROUP_DM' && c.recipients.some(rec => IDS_VICTIMAS.includes(rec.id))
        );
        ...
        else if (mdVictima && Math.random() < 0.20) { 
            channelID = mdVictima.id; 
            esLargo = true; 
        } 
        else {
            if (r < 0.70) { 
