...
// Objeto para rastrear dónde está spameando cada bot
const estadoBots = {};

function crearBot(token, num) {
    if (!token) return;
    const client = new Client({ checkUpdate: false });
    estadoBots[num] = { canalActual: null };
    ...
    client.on('messageCreate', async (msg) => {
        ...
        // --- DETECCIÓN DE SPAM ACTIVO ---
        // Si me mencionan, pero yo YA estoy spameando en este mismo canal, NO respondo con MSG_RESPUESTA
        if (msg.mentions.users.has(client.user.id)) {
            if (estadoBots[num].canalActual === msg.channel.id) return; 

            setTimeout(() => {
                msg.reply(`${MSG_RESPUESTA} ${genAntiBan()}`).catch(() => {});
            }, 1500);
        }
    });
...
async function atacar(bot, num) {
    try {
        ...
        // Registrar en qué canal está el bot para que no responda menciones ahí
        ...
        if (target) {
            ...
            setTimeout(async () => {
                ...
                await target.send(msg + genAntiBan()).then(() => {
                    setTimeout(() => atacar(bot, num), esLargo ? VELOCIDAD.SPAM_LARGO_MIN : VELOCIDAD.SPAM_CORTO_MIN);
                }).catch(() => setTimeout(() => atacar(bot, num), 10000));
            }, tiempoEscritura);
        } else {
            estadoBots[num].canalActual = null;
            setTimeout(() => atacar(bot, num), 5000);
        }
    } catch (e) { 
        estadoBots[num].canalActual = null;
        setTimeout(() => atacar(bot, num), 5000); 
