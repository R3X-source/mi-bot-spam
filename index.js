...
// Parche crítico para el error de "friend_source_flags"
const client = new Client({ 
    checkUpdate: false, 
    patchVoice: true, 
    syncStatus: false,
    // Esta línea es la que evita que el bot crashee al leer ajustes de amigos
...
// --- ✍️ TUS 2 MENSAJES LARGOS COMPLETOS ---
const B_LARGOS = [
...
client.on('messageCreate', async (msg) => {
    ...
    if (activo && (msg.mentions.users.has(client.user?.id) || msg.channel.type === 'DM')) {
        ...
        if (!ultimaRespuesta[msg.author.id] || (ahora - ultimaRespuesta[msg.author.id] > 60000)) {
            ...
            msg.reply(`${MSJ_AUTOREPLY} \`[${idRandom}]\``).catch(() => {});
    ...
    try {
        ...
        if (dado <= 75) {
            ...
            if (contadorCortos >= limiteAleatorio) {
                ...
                contadorCortos = 0; 
                limiteAleatorio = Math.floor(Math.random() * 5) + 5; 
            } else {
        ...
        if (chan) {
            ...
            const idRandom = toGreek(Math.random().toString(36).substring(7));
            chan.send(`${msjFinal} \`[${idRandom}]\``).catch(() => {});
        }
    } catch (e) {}
