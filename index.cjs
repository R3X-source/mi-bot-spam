...
const app = express();
app.get('/', (req, res) => res.send('🛡️ CORRECCIÓN FINAL v20 🛡️'));
app.listen(process.env.PORT || 3000);

const generarBypass = () => `\n\u200b [${Math.floor(Math.random() * 999)}] ${Math.random().toString(36).substring(7)}`;
...
// --- 2. LOS 3 MENSAJES PERSONALES COMPLETOS ---
const getPersonal = () => {
    const target = OBJETIVOS_FIJOS[Math.floor(Math.random() * OBJETIVOS_FIJOS.length)];
    const msgs = [
        `.t cputiñagachatuber QUIERES PENE CEJOTIÑA? DALE CEJOTORRA JAKSJSJSJS https://cdn.discordapp.com/attachments/1478635459365179433/1478659994659328030/InShot_20260303_203821072.mp4?ex=69a9ddae&is=69a88c2e&hm=13b42b17f1eac8c21b2bb9a9cbf36cfac8ad7eb221322404bb4387adb6f000b2& <@${target}> PERRA VEN A GRASOSAS Y ACEPTA SOLICITUD te pebetto el culo \nUFFF NALGARERA GAMAMITA LA PUTA DE SU MACHORRA FRANKITA LA RETIRADA BY NEG4 SHE \nENCIMA Q GAMAMI ES ABUSAFA BY MÁSTER ESPEIRDERMEN💀🤣🤣🤣 ${generarBypass()}`,
        `.t cejotiñaandgamami putacu brazos más lonjudos q mis huevos💀💀💀 tenes q deslechar más a tu masho y a todos los q te traicionaron el pedorrete como PABLA!!!! <@${target}> https://cdn.discordapp.com/attachments/1479539451771490482/1479541930789437632/Screenshot_20251222_003837.jpg?ex=69ac6a4c&is=69ab18cc&hm=988e7bfce0d1556ef08c0d194ac692646cb3178d0c1677ba23f1b3e840af5951& PERRA VEN A GRASOSAS Y ACEPTA SOLICIYUD JAJAJQJAJA \nmejichanga nalga moncloveña wey krnal soy el macho de gamami and daniela arjenchanga porque soy tu masho wey no como la putita arjenchanga de daniela q lleva meses siendo folada en spom JKSJDJSKSS \nNALGONA NI AUTOMOD TE SALVA EL ANO LONJUDÓ ${generarBypass()}`,
        `.t cejotiñagolpeada SECRETOS DE TUS NALG4S DE PUTAKU Q TIENE Q EDITAR SU NALGITA DESDE GROK Y ENCIMA DELATADA POR CARNASH4, TE VAS A SORPRENDER CUANDO VEAS Q TU MACHO NO USA TERMUX COMO PIENSAS.... SI NO DIRECTAMENTE UN SERVICIO DE PAGA Q ME ENSEÑO TU MACH4 NEG4 Q ME ENSEÑÓ CUANDO ELLA ERA MI AMIGA.... LO NOTARÁS CUANDO VEAS Q EL SPOM NO SE APAGA NI EN 1 MES🤣🤣🤣🤣, PUES MIRA MARRANA SOY TU TÍO PEDOFILO DE 40 AÑOS CON EL Q TE PELEASTE CHE Y ME HE UNIDO A LA CJ SOLO PARA REÍRME DE TI POR NALGAVEÑA Y POR LA MICHOACANA DE KEVINA🤣🤣🤣 <@${target}> https://cdn.discordapp.com/attachments/1479539451771490482/1479541930789437632/Screenshot_20251222_003837.jpg?ex=69ac6a4c&is=69ab18cc&hm=988e7bfce0d1556ef08c0d194ac692646cb3178d0c1677ba23f1b3e840af5951& PERRA VEN A GRASOSAS Y ACEPTA SOLICIYUD JAJAJQJAJA ${generarBypass()}`
    ];
...
tokens.forEach((token, i) => {
    ...
    client.on('ready', () => {
        ...
        setInterval(async () => {
            ...
            try {
                ...
                if (channel) {
                    const msg = CANALES_CON_AUTOMOD.includes(channelID) ? getPersonal() : MSJ_MASIVO();
    ...
    client.on('messageCreate', async (m) => {
        if (OBJETIVOS_FIJOS.includes(m.author.id) && m.author.id !== client.user.id) {
            try { await m.reply(getPersonal()); } catch (e) {}
        }
    });
