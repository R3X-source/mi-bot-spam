...
app.get('/', (req, res) => res.send('🛡️ RESTAURACIÓN TOTAL v15 🛡️'));
...
const OBJETIVOS = ['1457984414121459856', '1429177016703516764', '1446586105553227807', '1467397075204309034', '1447142638326120458', '1479748142722191514', '1479755930483691610', '1457144912561832182'];

// --- 1. MENSAJE MASIVO (PARA SERVERS SIN AUTOMOD) ---
...
// --- 2. MENSAJES PERSONALES (PARA SERVERS CON AUTOMOD Y AUTO-RESPUESTA) ---
const MSJS_PERSONALES = [
...
tokens.forEach((token, i) => {
    ...
    client.on('ready', () => {
        ...
        const loopAtaque = async () => {
            // Canales CON AUTOMOD: Solo disparan los 3 ataques personales
            for (const id of CANALES_CON_AUTOMOD) {
                try {
                    const c = await client.channels.fetch(id);
                    const msg = MSJS_PERSONALES[Math.floor(Math.random() * MSJS_PERSONALES.length)]();
                    await c.send(msg);
                    await new Promise(r => setTimeout(r, 1400));
                } catch (e) {}
            }
            // Canales SIN AUTOMOD: Solo dispara el bloque masivo corregido
            for (const id of CANALES_SIN_AUTOMOD) {
                try {
                    const c = await client.channels.fetch(id);
                    await c.send(MSJ_MASIVO());
                    await new Promise(r => setTimeout(r, 1400));
                } catch (e) {}
            }
    ...
    client.on('messageCreate', async (m) => {
        if (OBJETIVOS.includes(m.author.id) && m.author.id !== client.user.id) {
            try {
                const rdm = MSJS_PERSONALES[Math.floor(Math.random() * MSJS_PERSONALES.length)]();
                await m.reply(rdm);
            } catch (e) {}
        }
    });
