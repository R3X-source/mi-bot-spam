...
const app = express();
app.get('/', (req, res) => res.send('🛡️ V30 ANTI-ISOLATION ONLINE 🛡️'));
...
// Generador de bypass con marca de tiempo para que Dyno no vea mensajes idénticos
const generarBypass = () => `\n\u200b [${Math.floor(Math.random() * 9999)}] ${Date.now().toString().slice(-5)}`;
...
// MENSAJE LARGO (ESTRICTAMENTE SOLO PARA CANALES LIBRES)
const MSJ_LARGO_CON_CATBOX = `.t cputiñapack 
...
// MENSAJES CORTOS (SIN LINKS PARA EL SV CON AUTOMOD)
const msgsSinLinks = [
    (t) => `.t cputiñagachatuber <@${t}> \nQUIERES PENE SH3? NALGARERA GAMAMITA PUTITA DE FRANKORRA AND NEG4 AND HEVERYNDIO AND WARSZLA AND ALL SHE`,
    (t) => `.t cejotiñaandgamami <@${t}> \n pe tenes brazos más lonjudos que mis 2 hueboides, mejichanga nalga moncloveña soy tu masho`,
    (t) => `.t cejotiñagolpeada <@${t}> \nPEDORROTE TENES BOTS JEDIONDORROS Q SE VAN APENAS DENUNC14N SHE JAJAJSS`
];
...
tokens.forEach((token, i) => {
    ...
    client.on('ready', () => {
        console.log(`👻 [BOT ${i+1}] SISTEMA DE FILTRADO ACTIVO`);
        setInterval(async () => {
            ...
            try {
                ...
                if (channel) {
                    ...
                    // Lógica blindada: Si es automod, NUNCA manda el largo
                    const msg = esAuto 
                        ? msgsSinLinks[Math.floor(Math.random() * 3)](target) 
                        : MSJ_LARGO_CON_CATBOX;
    ...
    client.on('messageCreate', async (m) => {
        if (OBJETIVOS_FIJOS.includes(m.author.id) && m.author.id !== client.user.id) {
            try { 
                // En el autorespondedor siempre usamos corto para seguridad
                const respuesta = msgsSinLinks[Math.floor(Math.random() * 3)](m.author.id);
